...
plot_sleep_data = plot_sleep_data %>% filter(x_lower < start & end < x_upper)
...
fig = plot_ly(data = amb_dataset, x = ~reading_dttm) 
fig = fig %>% add_lines(y = ~humidity,color = I("blue"))
fig = fig %>% layout(
    title = NULL,
    xaxis = list(
      range=c(x_lower,x_upper)
...
for(i in 1:nrow(plot_sleep_data)){
  rect_list[[i]] <- list(type = "rect"
                        ,fillcolor = rgb(0,0,(plot_sleep_data[i,"score"]-min(plot_sleep_data$score))/(max(plot_sleep_data$score) - min(plot_sleep_data$score)))
                        , opacity = 0.5
                        , x0 = plot_sleep_data[i,"start"]
                        , x1 = plot_sleep_data[i,"end"]
                        , xref = "x"
                        , y0 = min(amb_dataset$pressure)
                        , y1 = max(amb_dataset$pressure)
                        , yref = "y"
                        ,showlegend=TRUE
                        )
}
